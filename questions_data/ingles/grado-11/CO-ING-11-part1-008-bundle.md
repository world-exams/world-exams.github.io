---
id: "CO-ING-11-part1-008"
country: "CO"
grado: 11
asignatura: "ingles"
tema: "grammar-passive-voice"

total_questions: 7
estado: "draft"
creador: "Jules"
generation_date: "2025-11-30"
source: "OpenTDB"
source_license: "CC BY-SA 4.0"
---

# Pregunta Base: Grammar - Passive Voice

> **Fuente:** OpenTDB (CC BY-SA 4.0)


---

## Pregunta 1 (Original - Dificultad 3)
**ID:** `CO-ING-11-part1-008-v1`

### Enunciado
Choose the correct passive form of the sentence:
"The chef prepares the meal."

### Opciones
- [x] A) The meal is prepared by the chef.
- [ ] B) The meal was prepared by the chef.
- [ ] C) The meal is preparing by the chef.
- [ ] D) The meal has been prepared by the chef.

### Explicación Pedagógica
The active sentence is in Present Simple ("prepares"). The passive voice for Present Simple is formed with *am/is/are + past participle*. Therefore, "is prepared" is the correct form.


---

## Pregunta 2 (Fácil - Dificultad 1)
**ID:** `CO-ING-11-part1-008-v2`

### Enunciado
Complete the sentence with the correct form:
"The book _____ by the author."

### Opciones
- [x] A) is written
- [ ] B) writes
- [ ] C) writing
- [ ] D) write

### Explicación Pedagógica
We need the passive voice because the book receives the action. "Is written" is the correct passive structure (be + past participle).


---

## Pregunta 3 (Fácil - Dificultad 1)
**ID:** `CO-ING-11-part1-008-v3`

### Enunciado
Choose the correct passive sentence.

### Opciones
- [x] A) The car was washed by my dad.
- [ ] B) The car washed by my dad.
- [ ] C) The car is wash by my dad.
- [ ] D) The car was washing by my dad.

### Explicación Pedagógica
Option A uses the correct structure for Past Simple Passive: *was/were + past participle* ("was washed").


---

## Pregunta 4 (Media - Dificultad 3)
**ID:** `CO-ING-11-part1-008-v4`

### Enunciado
Change the sentence to passive voice:
"They are building a new bridge."

### Opciones
- [x] A) A new bridge is being built.
- [ ] B) A new bridge is built.
- [ ] C) A new bridge was being built.
- [ ] D) A new bridge has been built.

### Explicación Pedagógica
The active sentence is in Present Continuous ("are building"). The passive form is *am/is/are + being + past participle*. Thus, "is being built" is correct.


---

## Pregunta 5 (Media - Dificultad 3)
**ID:** `CO-ING-11-part1-008-v5`

### Enunciado
Change the sentence to passive voice:
"Someone stole my bike yesterday."

### Opciones
- [x] A) My bike was stolen yesterday.
- [ ] B) My bike is stolen yesterday.
- [ ] C) My bike has been stolen yesterday.
- [ ] D) My bike was stealing yesterday.

### Explicación Pedagógica
The active sentence is in Past Simple ("stole"). The passive form is *was/were + past participle*. "Was stolen" fits the past context ("yesterday").


---

## Pregunta 6 (Difícil - Dificultad 5)
**ID:** `CO-ING-11-part1-008-v6`

### Enunciado
Choose the correct passive form for:
"They will have finished the project by Monday."

### Opciones
- [x] A) The project will have been finished by Monday.
- [ ] B) The project will be finished by Monday.
- [ ] C) The project will have finished by Monday.
- [ ] D) The project would be finished by Monday.

### Explicación Pedagógica
The active sentence is in Future Perfect ("will have finished"). The passive structure is *will have been + past participle*. Option A follows this rule correctly.


---

## Pregunta 7 (Difícil - Dificultad 5)
**ID:** `CO-ING-11-part1-008-v7`

### Enunciado
Select the sentence that correctly uses the passive voice with a modal verb.

### Opciones
- [x] A) The application must be submitted before the deadline.
- [ ] B) The application must submit before the deadline.
- [ ] C) The application must be submitting before the deadline.
- [ ] D) The application must have submit before the deadline.

### Explicación Pedagógica
Passive with modals follows the structure *modal + be + past participle*. "Must be submitted" is the correct form. Option B is active, and C/D are grammatically incorrect.


| Total Preguntas | 7 |
| Original (Dificultad 3) | 1 |
| Fácil (Dificultad 1-2) | 2 |
| Media (Dificultad 3) | 2 |
| Difícil (Dificultad 4-5) | 2 |

# === METADATA GLOBAL ===

# === Metadata de Validación ===

| Campo | Valor |

---|
| Total Preguntas | 7 |
| Original (Dificultad 3) | 1 |
| Fácil (Dificultad 1-2) | 2 |
| Media (Dificultad 3) | 2 |
| Difícil (Dificultad 4-5) | 2 |

## 📊 Metadata de Validación

| Pregunta | ID | Dificultad | Validado |

---|
---|
---|
| 1 | CO-ING-11-part1-008-v1 | 3 | ⬜ |
| 2 | CO-ING-11-part1-008-v2 | 1 | ⬜ |
| 3 | CO-ING-11-part1-008-v3 | 1 | ⬜ |
| 4 | CO-ING-11-part1-008-v4 | 3 | ⬜ |
| 5 | CO-ING-11-part1-008-v5 | 3 | ⬜ |
| 6 | CO-ING-11-part1-008-v6 | 5 | ⬜ |
| 7 | CO-ING-11-part1-008-v7 | 5 | ⬜ |

